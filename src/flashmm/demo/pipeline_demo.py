...
class PipelineDemo:
    ...
    def __init__(self):
        ...
        self.metrics_collector: EnhancedMetricsCollector | None = None
        self.running = False
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # Register alert callback - handle async callback properly
            def alert_wrapper(alert_type: str, alert_data: dict[str, Any]) -> None:
                asyncio.create_task(self._on_alert(alert_type, alert_data))

            self.metrics_collector.register_alert_callback(alert_wrapper)
    ...
    async def _display_live_stats(self) -> None:
        ...
        try:
            # Get current metrics
            if self.market_data_service:
                await self.market_data_service.get_performance_metrics()
            if not self.metrics_collector:
                return
    ...
    async def _display_final_stats(self) -> None:
        ...
        try:
            if not self.demo_start_time:
                logger.warning("Demo start time not recorded")
                return
            ...
            if self.latency_measurements:
                ...
                target_violations = len([latency for latency in self.latency_measurements if latency > 250])
            ...
            try:
                if not self.market_data_service:
                    logger.warning("Market data service not available for health check")
                    return
