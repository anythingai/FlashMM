...
from flashmm.utils.decorators import measure_latency, require_trading_enabled
...
class QuotingStrategy:
    ...
    def __init__(self):
        ...
        self.last_prediction: PredictionSignal | None = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise TradingError(f"Strategy initialization failed: {e}") from e
    ...
    async def _get_market_data(self, symbol: str) -> dict | None:
        ...
        try:
            ...
            try:
                ...
                logger.error("MarketDataService missing get_market_snapshot method")
                return None
    ...
    async def _enhance_market_data(self, market_data: dict, symbol: str) -> dict:
        ...
        try:
            ...
            bid_size = sum(Decimal(str(level[1])) for level in bids[:5]) or Decimal('0')
            ask_size = sum(Decimal(str(level[1])) for level in asks[:5]) or Decimal('0')
    ...
    async def _get_prediction(self, symbol: str, market_data: dict) -> PredictionSignal | None:
    ...
    async def generate_quotes(
        ...
        """Generate quotes for external consumers (public interface)."""
        try:
            ...
            # Convert prediction dict to PredictionSignal if provided
            ...
            if prediction:
                ...
                prediction_signal = await self._get_prediction(symbol, market_data)

            if not prediction_signal:
                return []

            # Generate quotes using internal method
            quote_levels = await self._generate_quotes(symbol, market_data, prediction_signal)

            # Convert to dictionary format for external consumers
            quotes = []
            for quote_level in quote_levels:
                quotes.append({
                    'side': quote_level.side,
                    'price': float(quote_level.price),
                    'size': float(quote_level.size),
                    'spread_bps': quote_level.spread_bps,
                    'confidence': quote_level.confidence.value,
        ...
        try:
            ...
            for symbol, quotes in self.current_quotes.items():
                symbol_metrics[symbol] = {
                    ...
                    "avg_confidence": len([q for q in quotes if q.confidence in [QuoteConfidence.HIGH, QuoteConfidence.VERY_HIGH]]) / len(quotes) if quotes else 0.0,
