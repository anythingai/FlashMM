...
from flashmm.utils.exceptions import TradingError, ValidationError
...
from flashmm.trading.quotes.quote_generator import QuoteGenerator, SpreadOptimizer
from flashmm.trading.execution.order_router import OrderRouter, Order
...
from flashmm.trading.state.state_machine import get_state_machine, TradingState
...
class MarketMakingEngine:
    ...
    def __init__(self):
        ...
        self.quoting_strategy: Optional[QuotingStrategy] = None
        self.quote_generator: Optional[QuoteGenerator] = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise TradingError(f"MarketMakingEngine initialization failed: {e}")
    ...
    async def _setup_state_callbacks(self) -> None:
        ...
        self.state_machine.add_emergency_condition(
            StateCondition(
    ...
    async def _on_emergency_stop(self, state: TradingState) -> None:
        ...
        if self.order_router:
            await self.order_router.cancel_all_orders("Emergency stop")
    ...
    async def start(self) -> None:
        ...
        try:
            await self.state_machine.start_trading(
                message="Starting MarketMakingEngine",
                user_id="system"
            )
        except Exception as e:
    ...
    async def stop(self) -> None:
        ...
        try:
            await self.state_machine.shutdown(
                message="Stopping MarketMakingEngine",
                user_id="system"
            )
        except Exception as e:
    ...
    async def pause(self) -> None:
        ...
        await self.state_machine.pause(
            message="Pausing MarketMakingEngine",
            user_id="system"
        )
    ...
    async def resume(self) -> None:
        ...
        await self.state_machine.resume(
            message="Resuming MarketMakingEngine", 
            user_id="system"
        )
    ...
    async def stop_trading(self) -> None:
        ...
        if self.order_router:
            await self.order_router.cancel_all_orders("Trading stopped")
    ...
    async def _main_trading_loop(self) -> None:
        ...
        while self.is_running and self.state_machine.is_trading_active():
        ...
        try:
            for symbol in self.symbols:
                if self.inference_engine:
                    prediction = await self.inference_engine.get_prediction(
                        symbol=symbol,
        ...
        try:
            ...
            for quote in quotes:
                order = Order(
                    ...
                    time_in_force=quote.get('time_in_force', 'GTC'),
                    metadata={
                        'source': 'market_maker',
                ...
                await self.order_book_manager.place_order(order)
    ...
    async def _manage_orders(self) -> None:
        ...
        try:
            ...
            await self.order_book_manager.sync_order_book_state()
            
            # Process any filled orders
            filled_orders = await self.order_router.get_filled_orders() if self.order_router else []
        ...
        try:
            ...
            if self.position_tracker:
                await self.position_tracker.process_trade(
                    ...
                    is_maker=order.metadata.get('is_maker', True)
                )
            ...
            for symbol in self.symbols:
                ...
                if not compliance['compliant']:
                    ...
                    if compliance['limit_utilization'] > 1.5:  # 150% of limit
                        await self.state_machine.emergency_stop(
                            f"Critical inventory violation for {symbol}"
                        )
                        return
                    elif compliance['limit_utilization'] > 1.0:  # 100% of limit
                        await self.state_machine.pause(
                            f"Inventory limit exceeded for {symbol}"
                        )
                        return
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        base_metrics.update({
            ...
            'trading_state': self.state_machine.get_current_state().value if self.state_machine else 'unknown',
...
_market_making_engine: Optional[MarketMakingEngine] = None
