...
from flashmm.utils.decorators import measure_latency, require_trading_enabled, timeout_async
...
class Order:
    ...
    # Metadata
    source: str = "flashmm"
    parent_order_id: str | None = None  # For order replacements
...
class OrderReconciler:
    ...
    async def _reconcile_single_order(self, order: Order, results: dict[str, Any]) -> None:
        ...
        if age_minutes > 1 and order.status == OrderStatus.ACTIVE:
            import secrets
            if secrets.randbelow(10) == 0:  # 10% chance of partial fill
                fill_size = order.remaining_size * Decimal(str(secrets.randbelow(40) / 100 + 0.1))
                fill = OrderFill(
...
class OrderRouter:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise OrderError(f"OrderRouter initialization failed: {e}") from e
    ...
    async def _place_order_from_dict(self, order_data: dict[str, Any]) -> Order | None:
    ...
    async def _replace_order(self, old_order_id: str, new_order_data: dict[str, Any]) -> tuple[str, Order] | None:
    ...
    def get_orders_for_symbol(self, symbol: str, status_filter: set[OrderStatus] | None = None) -> list[Order]:
    ...
    def get_filled_orders(self, since: datetime | None = None) -> list[Order]:
        """Get orders that have been filled since the specified time."""
        try:
            filled_orders = [
            ...
            if since:
                # Filter by orders filled since the specified time
                filled_orders = [
                    order for order in filled_orders
                    if any(fill.timestamp >= since for fill in order.fills)
                ]
