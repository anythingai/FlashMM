...
from flashmm.utils.exceptions import OrderError, ValidationError
from flashmm.utils.decorators import measure_latency, timeout_async
from flashmm.trading.execution.order_router import OrderRouter, Order, OrderStatus
...
class OrderBookState:
    ...
    our_best_bid: Optional[Decimal]
    our_best_ask: Optional[Decimal]
...
class OrderPerformanceMetrics:
    ...
    time_to_acknowledge_ms: Optional[float] = None
...
class OrderBookManager:
    ...
    def get_order_book_state(self, symbol: str) -> Optional[OrderBookState]:
        """Get current order book state for symbol."""
        return self.order_book_states.get(symbol)
    
    def get_performance_metrics(self, order_id: str) -> Optional[OrderPerformanceMetrics]:
        """Get performance metrics for an order."""
        return self.performance_metrics.get(order_id)
    
    def get_all_performance_metrics(self, symbol: Optional[str] = None) -> List[OrderPerformanceMetrics]:
