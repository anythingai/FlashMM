...
from flashmm.utils.exceptions import OptimizationError, ValidationError
...
class QuoteOptimizer:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise OptimizationError(f"QuoteOptimizer initialization failed: {e}")
    ...
    async def optimize_quotes(
        ...
        try:
            ...
            raise OptimizationError(f"Quote optimization failed: {e}")
    ...
    async def _build_market_metrics(
        ...
        if not bids or not asks:
            raise OptimizationError(f"No order book data for {symbol}")
        ...
        bid_depth = sum(Decimal(str(bid['size'])) for bid in bids[:5])
        ask_depth = sum(Decimal(str(ask['size'])) for ask in asks[:5])
        ...
        return MarketMetrics(
            ...
            num_market_makers=len(set(bid['size'] for bid in bids[:10])),
    ...
    def get_optimization_performance(self, symbol: Optional[str] = None) -> Dict[str, Any]:
...
_quote_optimizer: Optional[QuoteOptimizer] = None
