...
class EnhancedRiskManager:
    ...
    def __init__(self):
        ...
        self.circuit_breakers: CircuitBreakerSystem | None = None
        self.position_limits: PositionLimitsManager | None = None
        self.market_monitor: MarketRiskMonitor | None = None
        self.pnl_controller: PnLRiskController | None = None
        self.operational_monitor: OperationalRiskManager | None = None
        self.risk_reporter: RiskReporter | None = None
        self.emergency_protocols: EmergencyProtocolManager | None = None
        ...
        self.pause_trading_callback: Callable | None = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise RiskError(f"EnhancedRiskManager initialization failed: {e}") from e
    ...
    def set_trading_engine_callbacks(self,
                                   ...
                                   pause_trading: Callable | None = None,
    ...
    async def perform_comprehensive_risk_check(self, symbol: str) -> RiskCheckResult:
        ...
        try:
            if self.enterprise_mode_enabled:
                ...
                if self.circuit_breakers:
                    system_halted = await self.circuit_breakers.check_all_breakers(market_data)
                    circuit_breakers_active = 1 if system_halted else 0

                    if system_halted:
                        violations.append("Circuit breaker system halted")
                ...
                if self.pnl_controller:
                    ...
                    pnl_analysis = await self.pnl_controller.assess_pnl_risk(
                        symbol=symbol,
                        current_pnl=Decimal(str(pnl_data.get('total_pnl', 0.0))),
                ...
                if self.emergency_protocols:
                    emergency_conditions = {
                        'components': {
                            ...
                            'market_risk': market_analysis,
                            'pnl_risk': pnl_analysis,
                            'operational_risk': ops_health
                        }
                    }
                ...
                metrics = {
                    ...
                    'market_risk_level': market_analysis.get('overall_risk_level', 'unknown'),
                    'pnl_risk_level': pnl_analysis.get('overall_risk_level', 'unknown'),
                    'system_health_score': ops_health.get('overall_score', 0),
                    'emergency_protocols_active': len(triggered_protocols) if isinstance(triggered_protocols, list) else (1 if triggered_protocols else 0)
                }
    ...
    async def _check_enhanced_position_limits(self, symbol: str) -> dict[str, Any]:
        ...
        try:
            # Get current positions as dict format
            positions_list = await self._handle_get_positions()
            positions_dict = {
                pos['symbol']: pos for pos in positions_list
    ...
    async def _update_enterprise_components_after_trade(self, symbol: str, side: str, size: float, price: float):
        ...
        try:
            ...
            if self.pnl_controller:
                _pnl_data = await self._get_current_pnl_data()
                await self.pnl_controller.update_pnl(
                    symbol,
                    Decimal(str(abs(size * price) if side == 'sell' else 0)),  # realized_pnl
                    Decimal('0'),  # unrealized_pnl
                    Decimal(str(size * price))  # position_value
                )
            ...
            if self.market_monitor:
                ...
                await self.market_monitor.add_market_data(symbol, trade_data[symbol])

            # Update position limits
            if self.position_limits:
                positions = await self._handle_get_positions()
                portfolio_value = Decimal('100000')  # Would calculate actual value
                # Convert list positions to dict format
                positions_dict = {
                    pos['symbol']: pos for pos in positions
                    if isinstance(pos, dict) and 'symbol' in pos
                }
                await self.position_limits.check_portfolio_limits(positions_dict, portfolio_value)
