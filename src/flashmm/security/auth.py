...
class AuthenticationManager:
    ...
    def rotate_api_key(self, old_key_id: str) -> Optional[str]:
    ...
    async def verify_api_key(self, api_key: str, source_ip: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if key_info["allowed_ips"] and source_ip not in key_info["allowed_ips"]:
                await logger.log_authentication_event(
                    ...
                    key_id,
                    False,
                    {"reason": "ip_not_allowed", "source_ip": source_ip}
                )
                return None
            ...
            await logger.log_authentication_event(
                ...
                key_id,
                True,
                {"user_id": key_info.get("user_id"), "role": key_info["role"].value}
            )
    ...
    async def require_permission(self, user_context: Dict[str, Any], permission: str) -> None:
        ...
        if not role:
            await logger.log_authorization_event(
                ...
                "permission_check",
                False,
                {"reason": "no_role", "session_id": session_id}
            )
        ...
        await logger.log_authorization_event(
            ...
            "permission_check",
            True,
            {"role": role, "session_id": session_id, "mfa_verified": mfa_verified}
        )
