...
from typing import Dict, Any, List, Optional, Set, Union, AsyncIterator
...
class AuditEvent:
    ...
    hash_chain_prev: Optional[str] = None  # Previous event hash for chain integrity
    hash_signature: Optional[str] = None  # Cryptographic signature of this event
...
class AuditStorage:
    ...
    async def store_event(self, event: AuditEvent) -> None:
        ...
        event_hash = self.integrity_manager.compute_event_hash(event)
    ...
    async def verify_integrity(self) -> Dict[str, Any]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            for row in rows:
                ...
                # Verify signature
                if not self.integrity_manager.verify_event_signature(event, event.hash_signature):
...
class ComplianceReporter:
    ...
    async def generate_compliance_report(self, 
        ...
        report_data["statistics"] = {
            ...
            "unique_actors": len(set(e.actor_id for e in all_events)),
            "components_accessed": len(set(e.component for e in all_events))
        }
...
class ForensicAnalyzer:
    ...
    async def investigate_incident(self, 
                                 ...
                                 start_time: datetime,
                                 end_time: datetime,
                                 related_actors: List[str] = None,
                                 related_resources: List[str] = None) -> Dict[str, Any]:
        ...
        for event in all_events:
            ...
            elif related_resources and event.resource in related_resources:
                relevant_events.append(event)
        ...
        for actor_id, actor_event_list in actor_events.items():
            ...
            if len(failed_events) >= 5:
                ...
                if rapid_failures:
                    patterns.append({
                        ...
                        "actor_id": actor_id,
                        "description": f"5 failed attempts within 5 minutes",
            ...
            if privilege_events:
                patterns.append({
                    ...
                    "actor_id": actor_id,
                    "description": f"Privilege-related operations detected",
                    "event_count": len(privilege_events),
                    "actions": list(set(e.action for e in privilege_events))
                })
            ...
            if len(off_hours_events) > len(actor_event_list) * 0.8:  # 80% off-hours activity
                patterns.append({
                    ...
                    "actor_id": actor_id,
                    "description": f"High percentage of off-hours activity",
    ...
    def _analyze_actor_behavior(self, events: List[AuditEvent]) -> Dict[str, Any]:
        ...
        for actor_id, actor_event_list in actor_events.items():
            ...
            unique_components = len(set(e.component for e in actor_event_list))
            unique_actions = len(set(e.action for e in actor_event_list))
            unique_resources = len(set(e.resource for e in actor_event_list if e.resource))
    ...
    async def _cleanup_old_data(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                # Define retention periods by event type
                retention_periods = {
