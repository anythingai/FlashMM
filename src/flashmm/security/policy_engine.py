...
class PolicyConditionEvaluator:
    ...
    def evaluate_condition(self, condition: dict[str, Any], context: dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Log evaluation error but don't fail open - using fail-safe approach
            return False
    ...
    def _op_greater_than(self, field_val: Any, threshold: int | float, context: dict[str, Any]) -> bool:
        try:
            return float(field_val) > float(threshold)
        except (ValueError, TypeError):
            return False

    def _op_less_than(self, field_val: Any, threshold: int | float, context: dict[str, Any]) -> bool:
        try:
            return float(field_val) < float(threshold)
        except (ValueError, TypeError):
            return False

    def _op_between(self, field_val: Any, range_vals: list[int | float], context: dict[str, Any]) -> bool:
...
class PolicyEnforcer:
    ...
    async def _action_custom(self, violation: PolicyViolation,
        ...
        custom_action = rule.metadata.get("custom_action") if rule.metadata else None
...
class PolicyEngine:
    ...
    async def evaluate_event(self, component: str, action: str, context: dict[str, Any],
                           user_id: str | None = None, resource: str | None = None) -> dict[str, Any]:
    ...
    async def _evaluate_policy(self, policy: SecurityPolicy, component: str, action: str,
                              context: dict[str, Any], user_id: str | None,
    ...
    async def _has_active_exception(self, policy_id: str, user_id: str | None,
    ...
    async def _policy_metrics_collector(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                # Skip updating timestamp as it's not part of the stats dict structure
                pass
