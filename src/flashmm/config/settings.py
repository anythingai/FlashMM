...
class Settings(BaseSettings):
    ...
    enable_trading: bool = Field(default=False, env="TRADING_ENABLED")
    
    # Sei network
    sei_rpc_url: str = Field(default="https://sei-testnet-rpc.polkachu.com", env="SEI_RPC_URL")
    sei_ws_url: str = Field(default="wss://sei-testnet-rpc.polkachu.com/websocket", env="SEI_WS_URL")
    sei_chain_id: str = Field(default="atlantic-2", env="SEI_CHAIN_ID")
    ...
    influxdb_org: str = Field(default="flashmm", env="INFLUXDB_ORG")
    ...
    cambrian_secret_key: Optional[str] = Field(default=None, env="CAMBRIAN_SECRET_KEY")
    sei_private_key: Optional[str] = Field(default=None, env="SEI_PRIVATE_KEY")
    ...
    # ML Model
    ml_model_path: str = Field(default="/app/models/latest.pt", env="ML_MODEL_PATH")
    ml_confidence_threshold: float = Field(default=0.6, env="ML_CONFIDENCE_THRESHOLD")
    ml_inference_timeout_ms: int = Field(default=5, env="ML_INFERENCE_TIMEOUT_MS")
    ...
    async def _load_environment_config(self) -> None:
        ...
        if os.path.exists(config_path):
            try:
                ...
                raise ConfigurationError(f"Failed to load environment config {config_path}: {e}")
    ...
    async def set_runtime_config(self, key: str, value: Any) -> None:
        ...
        if self.redis_client:
            try:
                ...
                raise ConfigurationError(f"Failed to set runtime config {key}: {e}")
        else:
