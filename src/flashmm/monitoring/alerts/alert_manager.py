...
class AlertManager:
    ...
    async def evaluate_metric(self, metric_name: str, current_value: float | str) -> list[Alert]:
    ...
    async def _evaluate_rule(self, rule: AlertRule, current_value: float | str) -> bool:
    ...
    def _evaluate_condition(self, condition: str, current_value: float | str, threshold: float | str) -> bool:
    ...
    async def _check_for_duration(self, rule: AlertRule, current_value: float | str) -> bool:
    ...
    async def _create_alert(self, rule: AlertRule, current_value: float | str) -> Alert | None:
        """Create a new alert."""
        try:
...
Tags: {', '.join([f'{k}={v}' for k, v in alert.tags.items()])}
    ...
    def get_active_alerts(self, severity_filter: list[AlertSeverity] | None = None) -> list[Alert]:
    ...
    async def send_alert(self, metric: str, severity: AlertSeverity, data: dict[str, Any]) -> None:
        """Send an alert based on metric and severity."""
        try:
            # Evaluate the metric against rules
            alerts = await self.evaluate_metric(metric, data.get('value', 0))

            # Process any triggered alerts
            if alerts:
                logger.info(f"Sent alert for metric {metric} with severity {severity.value}")

        except Exception as e:
            logger.error(f"Error sending alert for metric {metric}: {e}")

    async def cleanup(self) -> None:
        """Cleanup method called from other components."""
        await self.shutdown()
...
def create_threshold_alert_rule(metric_name: str, condition: str, threshold: float | str,
