...
class PerformanceAnalyzer:
    ...
    async def analyze_spread_improvement(self, market: str, 
                                       ...
                                       end_time: Optional[datetime] = None) -> SpreadAnalysis:
        ...
        try:
            ...
            current_mean_spread = np.mean(current_spreads)
            current_median_spread = np.median(current_spreads)
            spread_std = np.std(current_spreads)
            ...
            volume_weighted_spread = np.average(current_spreads, weights=volumes)
            ...
            statistical_significance = self._calculate_significance(current_spreads, baseline_spread)
            
            # Additional metrics
            bid_ask_data = await self._get_bid_ask_data(market, start_time, end_time)
            bid_improvement = self._calculate_bid_improvement(bid_ask_data, baseline_spread)
            ask_improvement = self._calculate_ask_improvement(bid_ask_data, baseline_spread)
            ...
            analysis = SpreadAnalysis(
                ...
                current_spread_bps=current_mean_spread,
                improvement_bps=improvement_bps,
                improvement_percent=improvement_percent,
                volume_weighted_improvement=volume_weighted_improvement,
                confidence_interval=confidence_interval,
                statistical_significance=statistical_significance,
                sample_size=len(spread_data),
                bid_improvement_bps=bid_improvement,
                ask_improvement_bps=ask_improvement,
                mid_price_stability=mid_price_stability,
    ...
    async def analyze_pnl_attribution(self, start_time: Optional[datetime] = None,
                                    end_time: Optional[datetime] = None) -> PnLAttribution:
        ...
        try:
            ...
            attribution = PnLAttribution(
                ...
                market_making_pnl=market_making_pnl,
                fees_paid=fees_paid,
    ...
    async def analyze_trading_efficiency(self, start_time: Optional[datetime] = None,
                                       end_time: Optional[datetime] = None) -> TradingEfficiency:
        ...
        try:
            ...
            if not trade_data:
                logger.warning(f"No trade data available for efficiency analysis")
    ...
    async def generate_performance_report(self, report_type: ReportType,
                                        ...
                                        markets: Optional[List[str]] = None) -> PerformanceReport:
    ...
    def _calculate_bid_improvement(self, bid_ask_data: List[Dict[str, Any]], baseline: float) -> float:
        ...
        bid_spreads = [(mid - bid) / mid * 10000 for bid, mid in zip(bid_prices, mid_prices)]
        avg_bid_spread = np.mean(bid_spreads)
        
        baseline_half = baseline / 2
        return baseline_half - avg_bid_spread
    ...
    def _calculate_ask_improvement(self, bid_ask_data: List[Dict[str, Any]], baseline: float) -> float:
        ...
        ask_spreads = [(ask - mid) / mid * 10000 for ask, mid in zip(ask_prices, mid_prices)]
        avg_ask_spread = np.mean(ask_spreads)
        
        baseline_half = baseline / 2
        return baseline_half - avg_ask_spread
    ...
    def _calculate_mid_price_stability(self, bid_ask_data: List[Dict[str, Any]]) -> float:
        ...
        return max(0.0, 1.0 - avg_change * 1000)  # Scale for readability
    ...
    def _calculate_effective_spread(self, spread_data: List[Dict[str, Any]]) -> float:
        ...
        return np.average(spreads, weights=volumes)
    ...
    def _calculate_average_fill_time(self, order_data: List[Dict[str, Any]]) -> float:
        ...
        return np.mean(fill_times) if fill_times else 0.0
    ...
    def _calculate_data_quality_score(self, spread_analyses: List[SpreadAnalysis],
        ...
        # Sample size quality
        if spread_analyses:
            avg_sample_size = np.mean([a.sample_size for a in spread_analyses])
