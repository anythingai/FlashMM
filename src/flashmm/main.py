...
class FlashMMApplication:
        ...
        try:
            ...
            if self.audit_logger:
                try:
                    ...
                    logger.debug(f"Audit logging failed during initialization error: {e}")
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # Start web server
            # Get host configuration with secure defaults
            host = self.config.get("api.host", "127.0.0.1")

            # Log security warning if binding to all interfaces
            if host == "0.0.0.0":  # noqa: S104 - intentionally checking for this security issue
                logger.warning(
                    "API server binding to all interfaces (0.0.0.0). "
                    "Ensure this is intended and properly secured with firewalls."
                )
        ...
        try:
            ...
            if self.emergency_manager:
                try:
                    ...
                    logger.debug(f"Emergency manager failed during shutdown: {e}")
