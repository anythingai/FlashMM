...
try:
    from cambrian_sdk import CambrianClient  # type: ignore
    from cambrian_sdk.exceptions import CambrianAPIError  # type: ignore
    from cambrian_sdk.types import Order as CambrianOrder  # type: ignore
    from cambrian_sdk.types import OrderSide  # type: ignore
    from cambrian_sdk.types import OrderType as CambrianOrderType  # type: ignore
...
from flashmm.utils.decorators import measure_latency, retry_async, timeout_async
from flashmm.utils.exceptions import BlockchainError, OrderError, ValidationError
...
class BlockchainOrderManager:
    ...
    def __init__(self, sei_client: SeiClient, market_config_manager: MarketConfigManager):
        ...
        # Cambrian SDK client
        self.cambrian_client: CambrianClient | None = None
    ...
    async def submit_order_to_blockchain(self, order: Order) -> bool:
        ...
        try:
            ...
            self.failed_submissions += 1
            raise OrderError(f"Blockchain order submission failed: {e}") from e
    ...
    async def _submit_via_cambrian(self, blockchain_order: dict[str, Any]) -> str | None:
        ...
        try:
            ...
            raise OrderError(f"Cambrian order submission failed: {e}") from e
    ...
    async def _query_blockchain_order_status(self, blockchain_order_id: str) -> dict[str, Any] | None:
        """Query order status from blockchain."""
        if not self.cambrian_client:
            # Mock implementation for development
            return {
    ...
    async def _process_order_fills(self, mapping: BlockchainOrderMapping, fills: list[dict[str, Any]]) -> None:
        ...
        try:
            for fill_data in fills:
                # Create OrderFill object with all required parameters
                fill = OrderFill(
    ...
    def get_order_mapping(self, order_id: str) -> BlockchainOrderMapping | None:
        """Get blockchain mapping for internal order ID."""
        return self.order_mappings.get(order_id)

    def get_internal_order_id(self, blockchain_order_id: str) -> str | None:
