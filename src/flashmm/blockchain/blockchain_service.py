...
from flashmm.blockchain.transaction_manager import (
    TransactionManager,
)
from flashmm.config.settings import get_config
from flashmm.monitoring.alerts.alert_manager import AlertSeverity
from flashmm.trading.execution.order_router import Order
from flashmm.utils.decorators import measure_latency, timeout_async
from flashmm.utils.exceptions import BlockchainError, ServiceError
...
class BlockchainService:
    ...
    def __init__(self):
        ...
        self.last_health_check: ServiceHealthCheck | None = None

        # Core components
        self.sei_client: SeiClient | None = None
        self.market_config_manager: MarketConfigManager | None = None
        self.order_manager: BlockchainOrderManager | None = None
        self.transaction_manager: TransactionManager | None = None
        self.account_manager: AccountManager | None = None
        self.blockchain_monitor: BlockchainMonitor | None = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise ServiceError(f"Blockchain service initialization failed: {e}") from e
    ...
    async def _initialize_market_config_manager(self) -> None:
        ...
        if not self.sei_client:
            raise BlockchainError("Sei client not initialized")
        ...
        try:
            ...
            raise BlockchainError(f"Market config manager initialization failed: {e}") from e
    ...
    async def _initialize_account_manager(self) -> None:
        ...
        try:
            ...
            raise BlockchainError(f"Account manager initialization failed: {e}") from e

    async def _initialize_order_manager(self) -> None:
        """Initialize blockchain order manager."""
        if not self.sei_client or not self.market_config_manager:
            raise BlockchainError("Dependencies not initialized")
        ...
        try:
            ...
            raise BlockchainError(f"Order manager initialization failed: {e}") from e
    ...
    async def _initialize_blockchain_monitor(self) -> None:
        ...
        try:
            ...
            raise BlockchainError(f"Blockchain monitor initialization failed: {e}") from e
    ...
    async def _setup_component_connections(self) -> None:
        ...
        try:
            ...
            if self.blockchain_monitor:
                ...
                def sync_alert_callback(metric: str, severity: AlertSeverity, data: dict[str, Any]) -> None:
                    asyncio.create_task(self._handle_blockchain_alert(metric, severity, data))

                self.blockchain_monitor.add_alert_callback(sync_alert_callback)
    ...
    async def submit_order_to_blockchain(self, order: Order) -> bool:
        ...
        try:
            if not self._is_service_ready() or not self.order_manager:
    ...
    def get_account_balance(self) -> dict[str, Any] | None:
        """Get current account balance information."""
        try:
            if not self.account_manager:
                return None

            asyncio.create_task(self.account_manager.get_active_account())
    ...
    async def _notify_status_change(self, new_status: ServiceStatus) -> None:
        ...
        for callback in self.status_change_callbacks:
            try:
                callback(new_status)  # These are sync callbacks
            except Exception as e:
    ...
    async def _notify_error(self, error_type: str, error: Exception) -> None:
        ...
        for callback in self.error_callbacks:
            try:
                callback(error_type, error)  # These are sync callbacks
            except Exception as e:
