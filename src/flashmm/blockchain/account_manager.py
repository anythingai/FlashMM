...
from flashmm.utils.decorators import timeout_async
from flashmm.utils.exceptions import BlockchainError, SecurityError
...
class AccountManager:
    ...
    def __init__(self, sei_client: SeiClient):
        ...
        self.current_active_account: str | None = None
    ...
    async def create_account(
        ...
        enabled_operations: set[str] | None = None
    ) -> AccountConfig:
    ...
    async def _create_account(
        ...
        try:
            ...
            wallet = LocalWallet(private_key)
            account_config.address = str(wallet.address())

            # Store encrypted private key (simplified for now)
            # In production, use proper key encryption through the key manager
            # For now, just store a placeholder since this is development mode
            await self._store_account_key(account_id, "encrypted_key_placeholder")
    ...
    async def get_active_account(self) -> AccountConfig | None:
    ...
    def get_account_details(self, account_id: str) -> dict[str, Any] | None:
