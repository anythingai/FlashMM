...
class TestVolatilityDetector:
    ...
    async def test_multiple_symbols(self, volatility_detector):
        ...
        for i, (symbol, base_price) in enumerate(zip(symbols, base_prices)):
            for j in range(15):
                price_data = {
                    'symbol': symbol,
...
class TestMarketRiskMonitor:
    ...
    async def test_comprehensive_market_analysis(self, market_monitor):
        ...
        analysis_result = await market_monitor.analyze_market_conditions(market_data)
    ...
    async def test_risk_threshold_monitoring(self, market_monitor):
        ...
        for i in range(25):
            ...
            await market_monitor.add_market_data(historical_data)
        
        # Analyze risky conditions
        analysis_result = await market_monitor.analyze_market_conditions(risky_market_data)
    ...
    async def test_multi_symbol_monitoring(self, market_monitor):
        ...
        # Analyze all symbols
        analysis_result = await market_monitor.analyze_market_conditions(multi_symbol_data)
...
async def test_market_stress_scenario():
    ...
    for i in range(30):
        ...
        await monitor.add_market_data(stable_data)
    
    # Analyze crash conditions
    crash_analysis = await monitor.analyze_market_conditions(crash_data)
