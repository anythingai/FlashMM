...
class TestStopLossManager:
    ...
    async def test_initialization(self, stop_loss_manager):
        ...
        assert stop_loss_manager.dynamic_adjustment_enabled
    ...
    async def test_stop_loss_execution(self, stop_loss_manager):
        ...
        _stop_loss_id = await stop_loss_manager.create_stop_loss(position_data)
...
async def test_integration_stress_scenario():
    ...
    risk_assessment = await controller.assess_pnl_risk(
        symbol="BTC-USD",
        current_pnl=crash_scenario['total_pnl'],
        position_value=Decimal('30000')  # Total notional value
    )
