        ...
        with patch.object(concentration_monitor, '_calculate_correlation', return_value=0.85):
            ...
            btc_eth_risk = next(
                (r for r in correlation_risks 
                 if set([r['symbol1'], r['symbol2']]) == set(['BTC-USD', 'ETH-USD'])),
                None
            )
...
async def test_integration_with_concentration_monitor():
    ...
    is_valid, reason = await manager.validate_position(risky_position)
    
    # Should be rejected due to concentration risk
    assert not is_valid
    assert 'concentration' in reason.lower() or 'sector' in reason.lower()
