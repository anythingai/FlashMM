...
class IntegratedRiskManager:
    ...
    async def perform_comprehensive_risk_check(self, market_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            # Handle case where cb_triggered might be bool or list
            if isinstance(cb_triggered, bool):
                triggered_count = 1 if cb_triggered else 0
                triggered_list = [cb_triggered] if cb_triggered else []
            else:
                triggered_count = len(cb_triggered) if cb_triggered else 0
                triggered_list = cb_triggered or []

            risk_assessment['components']['circuit_breakers'] = {
                'triggered_breakers': triggered_list,
                'active_breakers': triggered_count,
                'system_health': 'normal' if triggered_count == 0 else 'warning'
            }
            ...
            if 'pnl_data' in market_data:
                pnl_analysis = await self.pnl_controller.assess_pnl_risk(
                    market_data['pnl_data'].get('total_pnl', 0),
                    market_data.get('portfolio_value', Decimal('100000')),
                    market_data.get('portfolio_value', Decimal('100000'))  # position_value
                )
...
class TestBasicIntegration:
    ...
    async def test_market_stress_scenario(self, integrated_system):
        ...
        for _component_name, component_data in risk_assessment['components'].items():
...
async def test_end_to_end_trading_day_simulation():
    ...
    [event for event in daily_events if event['type'] == 'emergency_trigger']
