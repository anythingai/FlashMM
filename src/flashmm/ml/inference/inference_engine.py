...
from flashmm.utils.exceptions import ModelError, InferenceError
...
class InferenceEngine:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise ModelError(f"InferenceEngine initialization failed: {e}")
    
    @timeout_async(0.45)  # 450ms timeout for Azure OpenAI integration
    @measure_latency("ml_inference")
    async def predict(self, market_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _convert_to_order_book_snapshot(self, order_book_dict: Dict[str, Any], symbol: str) -> OrderBookSnapshot:
        ...
        try:
            ...
            raise InferenceError(f"Order book conversion failed: {e}")
    ...
    def _convert_to_trades(self, trades_list: List[Dict[str, Any]], symbol: str) -> List[Trade]:
        ...
        try:
            ...
            raise InferenceError(f"Trade conversion failed: {e}")
    ...
    async def predict_with_method(self, 
        ...
        try:
            ...
            raise InferenceError(f"Prediction with method {method} failed: {e}")
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if self.prediction_service and self._initialized:
            try:
                service_stats = asyncio.create_task(self.prediction_service.get_performance_stats())
