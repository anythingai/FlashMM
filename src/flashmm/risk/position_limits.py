...
from flashmm.utils.exceptions import RiskError, ValidationError
...
class PositionLimitsManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise RiskError(f"Position limits manager initialization failed: {e}")
    ...
    async def check_position_limits(self, positions: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        async with self._lock:
            try:
                ...
                raise RiskError(f"Position limit check failed: {e}")
    ...
    def _calculate_compliance_score(self, violations: List[Dict], warnings: List[Dict]) -> float:
        ...
        total_limits = len([l for l in self.limits.values() if l.enabled])
        if total_limits == 0:
            return 1.0
