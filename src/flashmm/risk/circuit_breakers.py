...
class BaseCircuitBreaker:
    ...
    async def check_condition(self, value: float, metadata: dict[str, Any] | None = None) -> bool:
...
class PriceCircuitBreaker(BaseCircuitBreaker):
    ...
    def __init__(self, symbol: str, config: BreakerConfig | None = None):
        if config is None:
    ...
    async def _should_trip(self, value: float, metadata: dict[str, Any]) -> bool:
        ...
        price_change_pct = value  # value represents price_change_pct for this breaker
        ...
        if current_price > 0:
            ...
            if len(self.price_history) >= 10:
                ...
                self.volatility_estimate = float(np.std(returns) * np.sqrt(288))  # Scale to daily (5min intervals)

        # Calculate z-score
        z_score = abs(price_change_pct) / (self.volatility_estimate * 100)  # Convert to percentage

        return bool(z_score > self.config.threshold)
...
class VolumeCircuitBreaker(BaseCircuitBreaker):
    ...
    async def _should_trip(self, value: float, metadata: dict[str, Any]) -> bool:
        ...
        current_volume = value  # value represents current_volume for this breaker
        ...
        if len(self.volume_history) >= 10:
            ...
            self.normal_volume_estimate = float(np.median(self.volume_history[:-1]))  # Exclude current

        # Check volume ratio
        if self.normal_volume_estimate > 0:
            volume_ratio = current_volume / self.normal_volume_estimate
            return bool(volume_ratio > self.config.threshold)
...
class PnLCircuitBreaker(BaseCircuitBreaker):
    ...
    async def _should_trip(self, value: float, metadata: dict[str, Any]) -> bool:
        ...
        current_pnl = value  # value represents current_pnl for this breaker
...
class LatencyCircuitBreaker(BaseCircuitBreaker):
    ...
    async def _should_trip(self, value: float, metadata: dict[str, Any]) -> bool:
        ...
        current_latency_ms = value  # value represents current_latency_ms for this breaker
        ...
        if len(self.latency_history) >= 10:
            self.baseline_latency = float(np.percentile(self.latency_history[:-5], 50))  # 50th percentile

        # Trip if current latency is much higher than baseline
        return bool(current_latency_ms > max(self.config.threshold, self.baseline_latency * 5))
...
class CircuitBreakerSystem:
    ...
    def __init__(self):
        ...
        self.halt_reason = ""
        self.halt_time: datetime | None = None
        ...
        self.reduce_position_callback: Callable | None = None
        self.widen_spreads_callback: Callable | None = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise RiskError(f"Circuit breaker system initialization failed: {e}") from e
    ...
    def set_callbacks(self,
                      ...
                      reduce_position: Callable | None = None,
                      widen_spreads: Callable | None = None) -> None:
    ...
    def set_emergency_callback(self, callback: Callable) -> None:
        """Set emergency callback function."""
        self.halt_trading_callback = callback
    ...
    async def emergency_stop(self, reason: str = "Manual emergency stop") -> None:
        ...
        async with self._lock:
            self.system_halted = True
            self.halt_time = datetime.now()
            self.halt_reason = f"Emergency stop: {reason}"
            ...
            for breaker in self.breakers.values():
                if breaker.state != BreakerState.OPEN:
                    await breaker.manual_trip(f"Emergency stop: {reason}")

            # Execute emergency callback
            if self.halt_trading_callback:
                try:
                    await self.halt_trading_callback(reason)
                except Exception as e:
