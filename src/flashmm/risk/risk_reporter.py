...
class RiskDataCollector:
    ...
    async def collect_all_risk_data(self) -> dict[str, Any]:
        ...
        if 'market_risk' in self.risk_components:
            try:
                self.risk_components['market_risk']
...
class RiskDashboard:
    ...
    async def generate_dashboard_data(self) -> RiskDashboardData:
        ...
        try:
            ...
            raise RiskError(f"Dashboard generation failed: {e}") from e
    ...
    async def _calculate_position_risk_score(self, risk_data: dict[str, Any]) -> float:
        ...
        try:
            ...
            for _limit_name, limit_info in limits.items():
...
class ComplianceReporter:
    ...
    async def generate_daily_compliance_report(self, report_date: datetime) -> ComplianceReport:
        ...
        try:
            ...
            raise RiskError(f"Compliance report generation failed: {e}") from e
...
class RiskReporter:
    ...
    async def initialize(self):
        ...
        try:
            ...
            raise RiskError(f"Risk reporting initialization failed: {e}") from e
    ...
    async def generate_daily_summary_report(self, report_date: datetime | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            raise RiskError(f"Daily summary generation failed: {e}") from e
    ...
    async def generate_breach_notification(self, breach_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise RiskError(f"Breach notification generation failed: {e}") from e
    ...
    async def generate_comprehensive_report(self, risk_data: dict[str, Any]) -> dict[str, Any]:
        """Generate comprehensive risk report from risk data."""
        try:
            report = {
                ...
                'risk_summary': {
                    ...
                    'total_components': len(risk_data.get('components', {})),
                    'components_with_errors': len([c for c in risk_data.get('components', {}).values() if c.get('error')]),
                    'active_alerts': 0
                }
            }
            ...
            if 'circuit_breakers' in components:
                cb_data = components['circuit_breakers']
                if not cb_data.get('error'):
                    active_breakers = cb_data.get('active_breakers', 0)
                    if active_breakers > 0:
                        risk_indicators.append('circuit_breakers_active')
                        report['risk_summary']['overall_status'] = 'warning'

            # Check position limits
            if 'position_limits' in components:
                pos_data = components['position_limits']
                if not pos_data.get('error'):
                    violations = pos_data.get('total_violations', 0)
                    if violations > 0:
                        risk_indicators.append('position_limit_violations')
                        report['risk_summary']['overall_status'] = 'warning'

            # Check operational risk
            if 'operational_risk' in components:
                op_data = components['operational_risk']
                if not op_data.get('error'):
                    health = op_data.get('overall_health', 'good')
                    if health in ['critical', 'failure']:
                        risk_indicators.append('operational_issues')
                        report['risk_summary']['overall_status'] = 'critical'
                    elif health == 'warning':
                        risk_indicators.append('operational_warnings')
                        if report['risk_summary']['overall_status'] == 'healthy':
                            report['risk_summary']['overall_status'] = 'warning'

            report['risk_summary']['risk_indicators'] = risk_indicators
            report['risk_summary']['recommendation'] = (
                'Immediate attention required' if report['risk_summary']['overall_status'] == 'critical'
                else 'Monitor closely' if report['risk_summary']['overall_status'] == 'warning'
