    ...
    def should_trigger(self, conditions: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for condition in self.trigger_conditions:
                ...
                current_value = conditions.get(condition_type)
...
class PositionFlattener:
    ...
    async def flatten_all_positions(self, reason: str, emergency_level: EmergencyLevel) -> Dict[str, Any]:
        ...
        try:
            ...
            raise EmergencyStopError(f"Position flattening failed: {e}")
    ...
    async def _flatten_single_position(self, position: Dict[str, Any], emergency_level: EmergencyLevel) -> Dict[str, Any]:
        ...
        try:
            ...
            if emergency_level == EmergencyLevel.CATASTROPHIC:
                ...
                order_type = 'limit'
                price_tolerance = self.flatten_price_tolerance_pct
            else:
                # Conservative flattening
                order_type = 'limit'
                price_tolerance = 2.0  # 2% tolerance
...
class EmergencyProtocolManager:
    ...
    def __init__(self):
        ...
        self.place_order_callback: Optional[Callable] = None
        self.get_positions_callback: Optional[Callable] = None
        self.notify_stakeholders_callback: Optional[Callable] = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise RiskError(f"Emergency protocol manager initialization failed: {e}")
    ...
    def set_callbacks(self,
                      ...
                      place_order: Optional[Callable] = None,
                      get_positions: Optional[Callable] = None,
                      notify_stakeholders: Optional[Callable] = None):
    ...
    async def _execute_emergency_protocol(self, protocol: EmergencyProtocol, conditions: Dict[str, Any]):
        ...
        try:
            ...
            raise EmergencyStopError(f"Emergency protocol execution failed: {e}")
    ...
    async def manual_emergency_stop(self, reason: str, emergency_type: EmergencyType) -> Dict[str, Any]:
        ...
        try:
            ...
            raise EmergencyStopError(f"Manual emergency stop failed: {e}")
    ...
    async def emergency_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if action == ProtocolAction.HALT_TRADING:
                if self.halt_trading_callback:
                    await self.halt_trading_callback(f"Emergency: {emergency_event.trigger_reason}")
                return {
                    'action': action.value,
                    'status': 'completed',
                    'execution_time_seconds': time.perf_counter() - action_start,
            ...
            elif action == ProtocolAction.CANCEL_ORDERS:
                cancelled_count = 0
                if self.cancel_orders_callback:
                    result = await self.cancel_orders_callback("Emergency protocol")
                    cancelled_count = result.get('cancelled_count', 0)
                    emergency_event.orders_cancelled += cancelled_count
            ...
            elif action == ProtocolAction.FLATTEN_POSITIONS:
                flatten_result = await self.position_flattener.flatten_all_positions(
                    emergency_event.trigger_reason,
                    emergency_event.emergency_level
                )
                
                emergency_event.positions_affected += flatten_result.get('positions_flattened', 0)
                emergency_event.financial_impact += Decimal(str(flatten_result.get('total_financial_impact', 0)))
