...
from flashmm.utils.exceptions import AuthorizationError
...
class TestAuthenticationManager:
    ...
    async def test_lockout_mechanism(self, auth_manager):
        ...
        for _ in range(5):
            _result = await auth_manager.verify_api_key(test_key)
...
class TestAuthorizationManager:
    ...
    def test_role_permissions_retrieval(self, authz_manager):
        ...
        # Note: both_permissions variable is not defined in this scope
        # This assertion appears to be testing role permission inheritance
        # but the variable name suggests it should test both admin and readonly
        admin_permissions = authz_manager.get_role_permissions("admin")
        readonly_permissions = authz_manager.get_role_permissions("readonly")
        assert "config.read" in admin_permissions  # Admin should have read access
...
class TestAuthenticationIntegration:
    ...
    async def test_complete_auth_flow_with_mfa(self):
        ...
        admin_permissions = authz_manager.get_role_permissions("admin")
        permissions_list = list(admin_permissions) if isinstance(admin_permissions, set) else admin_permissions
    ...
    async def test_concurrent_session_management(self):
        ...
        session_tasks = []
        for i in range(10):
            async def create_session_task(user_index: int):
                return auth_manager.create_session(f"user_{user_index}", UserRole.ADMIN)

            task = asyncio.create_task(create_session_task(i))
