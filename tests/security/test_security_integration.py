...
from flashmm.security.audit_logger import AuditEventType, AuditLogger
from flashmm.security.auth import AuthorizationManager, UserRole
from flashmm.security.emergency_manager import EmergencyLevel, EmergencyManager, EmergencyType
from flashmm.security.key_manager import EnhancedKeyManager, KeySecurityLevel, KeyType
from flashmm.security.policy_engine import PolicyEngine
from flashmm.security.security_monitor import MonitoringEvent, SecurityMonitor
...
class TestSecuritySystemIntegration:
    ...
    async def test_emergency_response_coordination(self, security_system):
        ...
        _orchestrator = security_system["orchestrator"]
    ...
    async def test_key_management_security_integration(self, security_system):
        ...
        _monitor = security_system["monitor"]
        audit_logger = security_system["audit_logger"]
    ...
    async def test_cross_component_threat_correlation(self, security_system):
        ...
        _auth_event = await monitor.create_security_event(
            event_type=MonitoringEvent.LOGIN_ATTEMPT,
            component="authentication",
            action="login",
        ...
        _policy_result = await policy_engine.evaluate_event(
            component="web",
            action="admin_access",
            ...
            with patch.object(orchestrator.auth_manager, 'verify_api_key', return_value=None):
                for i in range(6):
                    try:
                        ...
                        # Expected authentication failures for testing
                        print(f"Expected auth failure {i+1}: {type(e).__name__}")
    ...
    def test_existing_permission_structure(self):
        ...
        for role_name in legacy_roles:
            try:
                ...
                next(r for r in UserRole if r.value == role_name)
        ...
        authz_manager.get_role_permissions("admin")
