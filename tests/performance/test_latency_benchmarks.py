...
class TestLatencyBenchmarks:
    ...
    async def test_data_normalization_latency(self, sample_sei_orderbook_message):
        ...
        print("Normalization Latency Stats:")
    ...
    async def test_websocket_message_processing_latency(self, sample_sei_orderbook_message):
        ...
        print("WebSocket Message Processing Latency Stats:")
        print(f"  Average: {avg_latency:.2f}ms")
        print(f"  95th percentile: {p95_latency:.2f}ms")
        print(f"  Maximum: {max_latency:.2f}ms")
    ...
    async def test_end_to_end_pipeline_latency(self, sample_sei_orderbook_message, sample_sei_trade_message):
        ...
        with patch('flashmm.data.market_data_service.HighPerformanceRedisClient', return_value=mock_redis_client), \
            ...
            for _ in range(50):
                ...
                elif normalized_data and isinstance(normalized_data, list) and normalized_data:
                    await service._process_market_data("SEI/USDC", normalized_data[0])
            ...
            print("End-to-End OrderBook Processing Latency:")
            ...
            print("End-to-End Trade Processing Latency:")
    ...
    async def test_memory_efficiency(self, sample_sei_orderbook_message):
        ...
        for _batch in range(10):  # 10 batches of 100 messages each
            tasks = []
            for _ in range(100):
                task = asyncio.create_task(normalizer.normalize_sei_data(sample_sei_orderbook_message))
                tasks.append(task)
    ...
    async def test_concurrent_symbol_processing(self):
        ...
        for _ in range(iterations):
            ...
            # Extract symbols from results safely
            result_symbols = set()
            for r in valid_results:
                if isinstance(r, dict) and "symbol" in r:
                    result_symbols.add(r["symbol"])
                elif isinstance(r, list) and r and isinstance(r[0], dict) and "symbol" in r[0]:
                    result_symbols.add(r[0]["symbol"])
    ...
    async def test_error_handling_performance_impact(self, sample_sei_orderbook_message):
        ...
        for i in range(message_count):
            ...
            # Mark result as intentionally unused for error case tracking
            _ = result
