...
from flashmm.monitoring.monitoring_service import MonitoringService, create_monitoring_service
from flashmm.monitoring.telemetry.metrics_collector import TradingMetrics, MLMetrics, RiskMetrics
...
from flashmm.monitoring.streaming.data_streamer import StreamType, MessageType
...
class MonitoringSystemValidator:
    ...
    async def setup_system(self) -> bool:
        ...
        try:
            ...
            health = self.monitoring_service.get_overall_health()
    ...
    async def run_end_to_end_test(self) -> IntegrationTestScenario:
        ...
        try:
            ...
            final_health = self.monitoring_service.get_overall_health()
    ...
    async def _publish_test_metrics(self) -> int:
        ...
        try:
            metrics_collector = self.monitoring_service.get_service("metrics_collector")
            ...
            for i in range(20):
                ml_metrics = MLMetrics(
                    ...
                    last_retrain_timestamp=datetime.now() - timedelta(hours=np.random.uniform(1, 24))
                )
    ...
    async def _verify_alert_processing(self) -> int:
        ...
        try:
            alert_manager = self.monitoring_service.get_service("alert_manager")
    ...
    async def _run_performance_analytics(self) -> Optional[Dict[str, Any]]:
        """Run performance analytics."""
        try:
            performance_analyzer = self.monitoring_service.get_service("performance_analyzer")
    ...
    async def _test_real_time_streaming(self) -> bool:
        ...
        try:
            data_streamer = self.monitoring_service.get_service("data_streamer")
    ...
    async def _generate_test_dashboards(self) -> int:
        ...
        try:
            dashboard_generator = self.monitoring_service.get_service("dashboard_generator")
            grafana_client = self.monitoring_service.get_service("grafana_client")
    ...
    async def _test_social_integration(self) -> bool:
        ...
        try:
            twitter_client = self.monitoring_service.get_service("twitter_client")
    ...
    async def _generate_streaming_load(self):
        ...
        for i in range(2000):  # 2000 stream messages
            stream_type = np.random.choice([StreamType.METRICS, StreamType.TRADES, StreamType.PNL])
    ...
    async def run_failure_recovery_test(self) -> IntegrationTestScenario:
        ...
        try:
            ...
            if service and hasattr(service, 'running'):
                ...
                failed_health = self.monitoring_service.get_overall_health()
                ...
                # Check recovery
                recovered_health = self.monitoring_service.get_overall_health()
    ...
    def generate_integration_report(self, scenarios: List[IntegrationTestScenario]) -> Dict[str, Any]:
        ...
        total_duration = sum((s.end_time - s.start_time).total_seconds() for s in scenarios if s.end_time)
        ...
        for scenario in scenarios:
            scenario_data = {
                ...
                "duration_seconds": (scenario.end_time - scenario.start_time).total_seconds() if scenario.end_time else 0,
