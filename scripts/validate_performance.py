...
class PerformanceValidator:
    ...
    def __init__(self):
        ...
        self.engine: MarketMakingEngine | None = None
    ...
    async def _validate_system_stability(self) -> None:
        ...
        while time.time() - start_time < self.test_duration_seconds:
            ...
            stability_metrics.append({
                ...
                'trading_state': self.engine.state_machine.get_current_state().value if self.engine and self.engine.state_machine else 'unknown',
    ...
    async def _validate_component_integration(self) -> None:
        ...
        integration_success = (
            ...
            orders_placed > 0 and
            self.engine.state_machine.is_trading_active() if self.engine and self.engine.state_machine else False
        )
        ...
        self.results.append(PerformanceResults(
            ...
            details={
                ...
                'components_active': {
                    'state_machine': self.engine.state_machine is not None if self.engine else False,
                    'quote_generator': self.engine.quote_generator is not None if self.engine else False,
                    'order_router': self.engine.order_router is not None if self.engine else False,
                    'position_tracker': self.engine.position_tracker is not None if self.engine else False
                }
            }
        ))
    ...
    async def _validate_error_handling(self) -> None:
        ...
        if not self.engine or not self.engine.state_machine:
            logger.error("Engine not properly initialized for error handling test")
            return
