...
from flashmm.security import (
    ...
    # Data structures
    EmergencyType, EmergencyLevel, AuditEventType, MonitoringEvent,
    KeyType, KeySecurityLevel, TestCategory
)
...
class SecurityIntegrationDemo:
    ...
    async def demonstrate_authentication_flow(self):
        ...
        api_key = self.orchestrator.auth_manager.generate_api_key(
            user_id="demo_admin",
            role=self.orchestrator.auth_manager.UserRole.ADMIN,
            requires_mfa=True
        )
        print(f"  ✓ Generated secure API key for demo_admin")
    ...
    async def demonstrate_integration_metrics(self):
        ...
        metrics = {
            "timestamp": datetime.utcnow().isoformat(),
            "orchestrator": self.orchestrator.get_security_status(),
            "monitor": self.monitor.get_monitoring_statistics(),
            "audit": self.audit_logger.get_audit_statistics(), 
            "emergency": self.emergency_manager.get_emergency_status(),
            "policies": self.policy_engine.get_policy_status(),
            "keys": self.key_manager.get_system_key_status(),
            "pentest": self.pentest_framework.get_framework_status()
        }
    ...
    async def test_system_resilience(self):
        ...
        test_key = self.orchestrator.auth_manager.generate_api_key(
            "load_test_user",
            self.orchestrator.auth_manager.UserRole.READONLY
        )
    ...
    async def run_complete_demo(self):
        ...
        try:
            ...
            final_metrics = await self.demonstrate_integration_metrics()
